// src/vTorrent.CLI/Commands/Torrent/ResumeCommand.cs
using System;
...
public static class ResumeCommand
{
    ...
    {
        var command = new Command("resume", "Resume a torrent");
    ...
    private static void AddArgumentsAndHandler(Command command)
    {
        ...
        command.SetAction(parseResult =>
        {
            ...
            using (client)
            {
                ...
                var result = client.ResumeAsync(hash).GetAwaiter().GetResult();
                if (!result.IsSuccess) { return CommandHelper.WriteApiError(result, formatter); }

                switch (formatter.Mode)
                {
                    case OutputMode.Json:
                        formatter.WriteJson(new { infoHash = hash, action = "resumed" });
                        break;
                    case OutputMode.Quiet:
                        formatter.WriteQuiet(hash);
                        break;
                    default:
                        formatter.WriteSuccess($"Resumed: {hash}");
                        break;
                }
            }
