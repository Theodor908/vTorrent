// src/vTorrent.CLI/Commands/Torrent/SuperSeedCommand.cs
using System;
...
public static class SuperSeedCommand
{
    private static readonly Argument<string> HashArgument = new("hash") { Description = "Torrent info hash" };

    public static Command Create()
    {
        var command = new Command("super-seed", "Toggle super-seeding mode");
    ...
    private static void AddArgumentsAndHandler(Command command)
    {
        ...
        command.SetAction(parseResult =>
        {
            ...
            using (client)
            {
                ...
                var result = client.ToggleSuperSeedAsync(hash).GetAwaiter().GetResult();
                if (!result.IsSuccess) { return CommandHelper.WriteApiError(result, formatter); }

                switch (formatter.Mode)
                {
                    case OutputMode.Json:
                        formatter.WriteJson(new { infoHash = hash, action = "toggled-super-seed" });
                        break;
                    case OutputMode.Quiet:
                        formatter.WriteQuiet(hash);
                        break;
                    default:
                        formatter.WriteSuccess($"Toggled super-seeding: {hash}");
                        break;
                }
            }
