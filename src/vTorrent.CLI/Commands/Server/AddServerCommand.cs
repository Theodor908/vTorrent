// src/vTorrent.CLI/Commands/Server/AddServerCommand.cs
using System;
...
public static class AddServerCommand
{
    public static Command Create()
    {
        ...
        {
            Description = "Username for this server",
            DefaultValueFactory = _ => "admin"
        };

        var command = new Command("add", "Add a server connection");
        ...
        command.SetAction(parseResult =>
        {
            ...
            if (json)
                formatter.WriteJson(new { server = name, host, https = !noHttps, insecure, username });
            else
                formatter.WriteSuccess($"Server '{name}' added");
