...
watch(() => props.visible, (v) => {
  if (v) { name.value = props.tag?.name ?? ''; }
});
...
async function handleSave(): Promise<void> {
  ...
  try {
    if (isEdit() && props.tag) {
      await tagsApi.updateTag(props.tag.id, { name: name.value.trim() });
      showToast('Tag updated.', 'success');
    } else {
      await tagsApi.createTag({ name: name.value.trim() });
...
async function handleDelete(): Promise<void> {
  ...
  saving.value = true;
  try {
    await tagsApi.deleteTag(props.tag.id);
...
<template>
  <Teleport to="body">
    <div v-if="visible" class="dialog-backdrop" @click.self="emit('close')">
      <div class="dialog" role="dialog" :aria-label="isEdit() ? 'Edit Tag' : 'New Tag'">
        <h3 class="dialog__title">{{ isEdit() ? 'Edit Tag' : 'New Tag' }}</h3>
        <div class="dialog__field">
          <label class="dialog__label" for="tag-name">Name</label>
          <input id="tag-name" v-model="name" class="dialog__input" type="text" placeholder="Tag name" />
        </div>
...
<style scoped>
.dialog-backdrop { position: fixed; inset: 0; z-index: 10000; background: rgba(0, 0, 0, 0.5); display: flex; align-items: center; justify-content: center; }
.dialog { background: var(--bg-card); border: 1px solid var(--border); border-radius: var(--radius-lg); padding: var(--spacing-xl); width: 100%; max-width: 400px; display: flex; flex-direction: column; gap: var(--spacing-lg); }
